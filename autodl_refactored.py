...
def trigger_download(show_name: str, magnet: str) -> bool:
    """
    Trigger a download by calling daisy_shell.sh.
    ...
    Returns:
        True if subprocess call succeeded, False otherwise
    """
    try:
        logger.info(f"Triggering download: {show_name}")
        subprocess.call([
            'sh',
            'daisy_shell.sh',
            'other',  # Type
            'autodl',  # Name (generic for auto-downloads)
            magnet
        ])
        return True
    except Exception as e:
