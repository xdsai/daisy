...
class FileOperations:
    ...
    def extract_subsplease_info(self, filename: str) -> Optional[Tuple[str, str, Optional[int]]]:
        """
        Extract show name and season from SubsPlease filename.

        Args:
            filename: SubsPlease filename (e.g., "[SubsPlease] Show Name S2 - 01 (1080p)")

        Returns:
            Tuple of (show_name, normalized_name, season_number) or None if not a SubsPlease file.
            season_number is None if no season suffix is found (defaults to Season 1).
        """
        ...
        try:
            ...
            if match:
                ...
                # Detect season suffix like "S2", "S3" etc.
                season_match = re.search(r'\bS(\d+)$', show_name)
                season = int(season_match.group(1)) if season_match else None
                normalized_name = self.normalize_name(show_name)
                logger.info(f"Extracted SubsPlease info: {show_name} -> {normalized_name} (season={season})")
                return show_name, normalized_name, season
        except Exception as e:
            logger.error(f"Failed to extract SubsPlease info from {filename}: {e}")

        return None

    def chown_to_user(self, path: str) -> bool:
        """
        Change ownership of a file/directory to the current user using sudo.
        ...
        try:
            ...
            logger.info(f"Changing ownership to {user}:{user}: {path}")
            subprocess.call(["sudo", "chown", f"{user}:{user}", path])
            return True
        except Exception as e:
    ...
    def move_show_files(
        ...
        Args:
            ...
            dest_base_path: Base path for shows (e.g., /mnt/media/shows/)
