    ...
    def download(
        ...
        try:
            ...
            if match:
                raw = match.group(1)
                if len(raw) == 32:
                    # Base32 encoded — decode to hex for qBittorrent comparison
                    infohash = binascii.hexlify(base64.b32decode(raw)).decode().upper()
                else:
                    infohash = raw.upper()
        ...
        try:
            ...
            # Re-login on 403 (session expired) and retry once
            if '403' in str(e):
                logger.warning("qBittorrent session expired, re-authenticating...")
                if self.connect():
                    try:
                        response = self.client.download_from_link(magnet, save_path=save_path)
                        logger.info(f"qBittorrent response after re-auth: {response}")
                    except Exception as retry_e:
                        logger.error(f"Exception starting download after re-auth: {retry_e}")
                        return None
                else:
                    logger.error("Failed to re-authenticate with qBittorrent")
                    return None
            else:
