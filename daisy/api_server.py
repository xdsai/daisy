...
def _infer_from_link(link: str) -> tuple:
    """
    Infer a name and media type from a magnet link or URL.

    Returns:
        (name, type) tuple. Either may be empty string if inference fails.
    """
    # SubsPlease show page URL
    # e.g. https://subsplease.org/shows/sousou-no-frieren-s2/
    if 'subsplease.org/shows/' in link:
        path = urlparse(link).path.rstrip('/')
        slug = path.split('/')[-1]  # "sousou-no-frieren-s2"
    ...
    if link.startswith('magnet:?'):
        ...
        if match:
            ...
            # For SubsPlease magnets, extract the show name
            sp_match = re.search(r'\[SubsPlease\]\s*(.+?)\s*-\s*\d+', dn)
            if sp_match:
                return sp_match.group(1), 'show'
            return dn, 'other'
...
def download():
    ...
    JSON body:
    - magnet: Magnet link or URL (required)
    - name: Show/movie name (optional for SubsPlease URLs and magnet links)
    - type: Media type (movie, show, other) - default: other (auto-set to 'show' for SubsPlease)
    ...
    try:
        ...
        # Auto-derive name and type when not provided
        if not name or not media_type:
            inferred_name, inferred_type = _infer_from_link(magnet)
            if not name:
                name = inferred_name
            if not media_type:
        ...
        if not name:
            ...
            return jsonify({'error': 'Missing name parameter (could not infer from link)'}), 400
